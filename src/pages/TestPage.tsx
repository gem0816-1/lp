...
type CreateSessionPayload = { sessionId: string };
type AnswerPayload = { completed: boolean };
...
export function TestPage() {
  ...
  useEffect(() => {
    async function bootstrap() {
      setLoading(true);
      const created = await apiJson<CreateSessionPayload>("/api/assessment/session", { method: "POST" });
      setSessionId(created.sessionId);
      const next = await apiJson<ProgressPayload>(`/api/assessment/${created.sessionId}/progress`);
      setProgress(next);
      setLoading(false);
    }
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    const result = await apiJson<AnswerPayload>("/api/assessment/answer", {
      method: "POST",
    ...
    const next = await apiJson<ProgressPayload>(`/api/assessment/${sessionId}/progress`);
    setProgress(next);
    ...
    <Space direction="vertical" size={20} style={{ display: "flex" }}>
      ...
      <Card>
        <Space direction="vertical" size={16} style={{ display: "flex" }}>
          <Space wrap>
            <Tag color="blue">
              第 {progress.question.questionIndex} / {progress.total} 题
            </Tag>
